...
import ScholarshipResolverView from './tools/scholarship-resolver/ScholarshipResolverView';
import ExplorerView from './tools/explorer/ExplorerView';
...
const HANDBOOK_SEED_PROMPT =
   'I need help as a Student Handbook Guide. Please answer my handbook questions clearly. If you need the exact handbook text, ask me to paste the relevant section. First question: What are the common rules and best practices a student should follow (attendance, discipline, academic integrity)?';
...
const Tools = () => {
   ...
   const tools = [
      ...
      {
         title: "Scholarship Resolver",
         desc: "Check likely scholarships based on your profile and see what's eligible, conditional, or blocked.",
         image: "https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&w=1200&q=60",
         action: "scholarship-resolver",
         badge: "Guided"
      },
      {
         title: "Degree & Career Explorer",
         desc: "Get degree suggestions and a career map based on your interests and goals.",
      ...
      },
      {
         ...
         desc: "Ask questions about student handbook rules, conduct, attendance, and academic integrity in chat.",
         image: "https://tse4.mm.bing.net/th/id/OIP.R9MTq5HRD_NBiR5sSRDqYgHaJl?rs=1&pid=ImgDetMain&o=7&rm=3",
         action: "handbook-guide",
         badge: "Chat"
      }
   ];
   ...
   if (activeTool === 'scholarship-resolver') return <ScholarshipResolverView onBack={handleBack} />;
   if (activeTool === 'explorer') return <ExplorerView onBack={handleBack} />;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
               {tools.map((tool, i) => (
                  <div
                     key={i}
                     onClick={() => {
                        if (tool.action === 'handbook-guide') {
                           navigate('/chat', { state: { seedPrompt: HANDBOOK_SEED_PROMPT } });
                           return;
                        }
                        setSearchParams({ tool: tool.action });
                     }}
                     className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 hover:border-primary-500 hover:shadow-premium transition-all duration-500 cursor-pointer group flex flex-col md:flex-row overflow-hidden reveal"
                  >
