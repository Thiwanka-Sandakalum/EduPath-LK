...
export async function listPrograms(req: Request, res: Response, next: NextFunction) {
    try {
        const { page = 1, limit = 20, institution_id, name, level, duration_value, delivery_mode, eligibility, sort = 'name:asc' } = req.query;
        const filter: any = {};
        if (institution_id) filter.institution_id = institution_id;
        if (name) filter.name = { $regex: name, $options: 'i' };
        if (level) filter.level = level;
        if (duration_value) filter['duration.value'] = +duration_value;
